...
char publicVotes[200]; //to store information of votes given by each voter
//----------------------------------------------------------------
...
int extractAadharNo(char userID[15])
{
    int aadharno=0;
    char tmp;
    for(int i=9;i<14;i++){
        tmp=userID[i];
		aadharno=(aadharno*10)+(tmp-48);
    }
    return aadharno;
}
...
void banID(){
    ...
    printf("Just Enter last four digit of aadhar no to ban\nPress 0 to exit... ");
    int input;
    while(1){
        printf("\nEnter Number: ");
        scanf("%d",&input);
...
void deleteIllegalVote(char userID[15])
{
    ...
    int location = extractAadharNo(userID);
    sprintf(filename,"candidate%d.txt",candidateArray[publicVotes[location-1]-49].cid);
    candidateArray[publicVotes[location-1]-49].votes--;
    publicVotes[location-1]='0';
...
void loadElectionInfoFromFile()
{
    ...
    //load candidates info and public votes
    for (int i = 0; i < currentValidID.totalVoters; i++)
    {
        publicVotes[i] = '0';
    }
...
int isValid(char userID[15])
{
    ...
    int inputedAadharNo = extractAadharNo(userID);
    
    if(inputedYear!=currentValidID.year || checkBranchCode(userID)!=1 || inputedAadharNo>currentValidID.totalVoters)
        return 0;
...
void saveVote(char userID[15],char voteInput)
{
    ...
    int location=extractAadharNo(userID);
    publicVotes[location-1]=voteInput;
...
void publicPanel()
{
    ...
    while(1)
	{
        ...
        if(isVoted(userID)!=0)
        {
            printf("\n  Your PAN entered is already voted\n  Contact Admin for furthur query");
            getch();
            continue;
        }
